@php
    $selectedTags = collect($tags)->pluck('id')->all();
@endphp
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('employee.recruitment.update', $data->id) }}" method="PUT" id="form_vacancy">
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    for (let a = 0; a < data.length; a++) {
                        ...
                        div += `
                            <div class="tag ${selected}" data-active="${dataActive}" id="tag-${data[a].id}" onclick="chooseTag(${data[a].id}, '${data[a].name['en']}')">${data[a].name['en']}</div>
                        `;
                    }
                    div += `
                    ...
                    // set selected tags
                    selectedTags = res.selected_tags;
                },
                error: function(err) {
